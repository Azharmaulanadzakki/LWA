            ...
            <form action="{{ route('tool.update', $tool->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" class="w-full border rounded-md py-2 px-3" rows="4">{{ $tool->description }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="tautan" id="tautan" class="w-full border rounded-md py-2 px-3" rows="4">{{ $tool->tautan }}</textarea>
                        @error('tautan')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex mt-6 justify-start">
                    <button type="submit"
                        class="ring-2 ring-slate-700 font-semibold px-6 py-2 leading-5 text-gray-900 hover:text-white transition-colors duration-200 transform bg-white rounded-md hover:bg-gray-700 focus:outline-none focus:bg-gray-900">Update</button>
                </div>
            </form>
        </section>
